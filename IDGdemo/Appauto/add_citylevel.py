...
def add_city_level():
    ...
    cursor = db.cursor()
    inqury_sql = """select storeId,city from Luckin_shop_detail"""
    ...
    for store_city in store_city_list:
        ...
        if city_level:
            update_sql = """update SHOP_DETAIL set city_level = '%s' where storeId = '%s'""" % (city_level, storeId)
            cursor.execute(update_sql)
        else:
            update_sql = """update SHOP_DETAIL set city_level = '%s' where storeId = '%s'""" % ('中国大陆外', storeId)
            cursor.execute(update_sql)
