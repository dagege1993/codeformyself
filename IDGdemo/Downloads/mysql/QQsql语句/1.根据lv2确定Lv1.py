...
lv_dict = {'办公商务': '软件', '实用工具': '软件', '便捷生活': '软件', '出行': '软件', '主题壁纸': '软件', '教育学习': '软件', '常用工具': '软件', '系统': '软件',
           ...
           '购物优惠': '软件', '金融理财': '软件', '微应用': '软件', '运动健康': '软件', '摄影摄像': '软件', '阅读': '软件', '音乐': '软件', '旅游': '软件',
           '娱乐': '软件', '安全': '软件', '健康': '软件', '教育': '软件', '新闻': '软件', '生活': '软件', '办公': '软件', '美化': '软件', '儿童': '软件',
           '导航': '软件', '工具': '软件', '腾讯软件': '软件',
           ...
           '日韩动漫': '游戏', '飞行射击': '游戏', '体育赛车': '游戏', '休闲益智': '游戏', '经营策略': '游戏', '棋牌中心': '游戏'}

db = pymysql.connect("192.168.103.31", "root", "adminadmin", "downloads")

# 使用 cursor() 方法创建一个游标对象 cursor
cursor = db.cursor()
sql = """select distinct category_lv2 from ANDROID_QQ_APP"""
cursor.execute(sql)
# 提交到数据库执行
...
for result in results:
    ...
    if lv1 != None:
        print(lv1, lv2, )
        up_sql = "UPDATE ANDROID_QQ_APP SET category_lv1 = '%s' WHERE category_lv2 = '%s'" % (lv1, lv2)
        print(up_sql)
