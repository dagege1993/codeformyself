...
def format_sql():
    inquery_sql = """select storeId,crawlTime from SHOP_WAITE  WHERE  crawlTime = '%s' GROUP BY storeId,crawlTime""" % day
    return inquery_sql
...
if __name__ == '__main__':
    ...
    not_data_list = ['2019-01-31', '2019-02-14', '2019-02-16']  # 不抓取列表,因为有服务器卡顿之类的原因,某些日期的数据是不能用的
    news_list = []  # 存储一天的数据
    ...
    filename = '海底捞校验门店数' + str(int(time.time())) + '.csv'
    with open(filename, 'w', newline='') as f:
        writer = csv.writer(f)
        head_list = ['店铺ID', '抓取时间', '店铺名称', '城市', '城市级别']
