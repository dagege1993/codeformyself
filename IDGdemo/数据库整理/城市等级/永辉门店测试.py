import pymysql
import pandas as pd


def add_city_level():
    db = pymysql.connect("192.168.103.31", "root", "adminadmin", "shops")
    cursor = db.cursor()
    inqury_sql = """select shop_name,city,address from YongHui"""
    cursor.execute(inqury_sql)
    city_level_dict = get_city_level()
    store_city_list = cursor.fetchall()
    for store_city in store_city_list:
        shop_name = store_city[0]
        city = store_city[1]
        city = city.split('市')[0]
        address = store_city[2]
        for city_dict in city_level_dict.keys():
            # if city_dict == "阜阳":
            if city_dict in city:   #
                city_level = city_level_dict.get(city_dict)
                if city_level:
                    update_sql = """update YongHui set city_level = '%s' where shop_name = '%s' and  address = '%s' """ % (
                        city_level, shop_name, address)
                    print(update_sql)
                    cursor.execute(update_sql)
                    break
                else:
                    print(shop_name, city, address)
                    update_sql2 = """update YongHui set city_level = '%s' where shop_name = '%s' and  address = '%s' """ % (
                        "四/五线城市", shop_name, address)
                    cursor.execute(update_sql2)


def get_city_level():
    path = r'C:\Users\Administrator.DESKTOP-DV7S27B\Desktop\需要维护的数据\城市等级.xlsx'
    results = pd.read_excel(path)
    city_list = results['城市']
    level_list = results['城市等级']
    city_level_dict = dict(zip(city_list, level_list))
    print(city_level_dict)
    return (city_level_dict)


if __name__ == '__main__':
    add_city_level()
