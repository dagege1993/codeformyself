...
for i, month_max_day in enumerate(month_list):
    ...
    sql = """select max(stat_dt),min(stat_dt) from ANDROID_SAFE_APP_DAILY where dt_type='周' and stat_dt>='%s-%s-01' and stat_dt<'%s'""" % (
        year,
        month, month_max_day)
    cursor.execute(sql)
...
""" % (end_time, start_time, start_time, end_time)
        ...
        df_data.to_csv('100APP_360.csv', mode='a', header=False, encoding="gbk", index=False)

# 最后再把文件转为xlsx,是为了利用可以追加写入csv
csv_data = pd.read_csv('100APP_360.csv', encoding="gbk")  # 再去读信息
csv_data.to_excel('100APP_360.xlsx', encoding="utf-8", index=False)
# 最后再删除csv文件
os.remove("100APP_360.csv")
