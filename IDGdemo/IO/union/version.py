...
cursor = db.cursor()
# SQL 插入语句
# sql = """Select * From VERSION Where DATE_FORMAT(finance_time,'%Y-%m-%d') >= '2018-10-01' and DATE_FORMAT(finance_time,'%Y-%m-%d') <= '2018-10-31' ORDER BY finance_time ;"""
sql = """Select * From VERSION Where DATE_FORMAT(finance_time,'%Y-%m-%d') >= '2018-11-01' and DATE_FORMAT(finance_time,'%Y-%m-%d') <= '2018-11-30' ORDER BY finance_time ;"""
try:
    # 执行sql语句
    cursor.execute(sql)
    # 提交到数据库执行
