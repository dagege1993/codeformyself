...
def storage(response):
    ...
    if response_text.get('message') == 'SUCCESS':
        ...
        for item in items:
            ...
            cursor = db.cursor()
            # SQL 插入语句
            sql = """INSERT INTO PENCILENEW(project_name,brief,industry,city,finance_time,finance,money,agency,legal_person,legal_name,registered_capital,competing_product,past_financing,teams,types,insert_times) VALUES ("%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%d","%s")""" % (
