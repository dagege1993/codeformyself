...
from selenium_from_mysql import get_path
from 便利日常更新下的全部文件 import xiniu

sql_list = []
start_time = time.time()
root_dir = get_path()  # 这里用的就是软编码,只要改一个地方
# root_dir = r'C:\Users\Administrator.DESKTOP-DV7S27B\Desktop\IDGdemo\selenium\日常更新/2019-03-02'

# root_dir = r'C:\Users\Administrator.DESKTOP-DV7S27B\Desktop\IDGdemo\selenium\日常更新'
print(root_dir)

lists = os.listdir(root_dir)  # 列出文件夹下所有的目录与文件
ll = [i for i in os.listdir(root_dir) if os.path.isdir(i)]
print(ll)
for i in range(0, len(lists)):
    ...
    if os.path.isfile(path):  # #判断路径是否为文件
        with open(path, 'r', encoding='utf-8') as f:  # 要读取非UTF-8编码的文本文件，需要给open()函数传入encoding参数
            ...
            registered_capital = sum([int(re.findall("\d+", capital)[0]) for capital in registered_capital])
            competing_products = page_sel.xpath('//*[@class="jsx-2833694364 table-section"]')
            competing_product_list = []
            for product in competing_products:
                # print(product.extract())
                product_source = scrapy.Selector(text=product.extract())
                competing_product = product_source.xpath(
                    '//*[@class="jsx-1418988208 name-link"]/text()').extract_first()
                if competing_product:
                    competing_product_list.append(competing_product)
            # 如果列表为空,就设置为空字符串
            if competing_product_list:
                competing_product_list = ','.join(competing_product_list)
            else:
                competing_product_list = ''
            ...
            cursor = db.cursor()
            # SQL 插入语句
            sql = "UPDATE unions1203 SET legal_name = '%s',legal_person= '%s',registered_capital= '%s',competing_product= '%s' WHERE project_name = '%s'" % (
