...
def get_company_names():
    ...
    cursor = db.cursor()
    sql = """Select project_name From unions1203 Where DATE_FORMAT(finance_time,'%Y-%m-%d') >= '2019-02-24'  AND legal_name = '' AND types=1 ORDER BY finance_time desc ;"""
    cursor.execute(sql)
    db.commit()
...
def seleniums():
    ...
    time.sleep(3)
    search_js = "document.getElementsByClassName('_3CdT1D4F9zbSLSraYABzJA')[0].click();"
    browser.execute_script(search_js)
    time.sleep(120)
    ...
    # browser.find_element_by_xpath('//*[@id="__next"]/div/div/nav/div/div[2]/div/div/input').send_keys(Keys.CONTROL, 'w')
