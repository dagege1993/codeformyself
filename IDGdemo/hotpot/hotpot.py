...
def get_shop_waite(shopId):
    ...
    for row in rowNumber:
        ...
        insert_sql = """insert ignore SHOP_WAITE (storeId,  waitNum,typeName,crawlTime,webque) VALUES ("%s","%s","%s","%s","%s") """ % (
...
def get_lost_shop():
    ...
    SELECT storeId FROM SHOP_WAITE WHERE crawlTime = '%s' GROUP by storeId
    UNION ALL
...
def get_lost_wait(storeId, key):
    ...
    if key == 1:
        ...
        response = requests.request("POST", url, data=payload, headers=headers, verify=False, proxies=proxies,
                                    timeout=6)
    if key == 2:
        response = requests.request("POST", url, data=payload, headers=headers, verify=False, timeout=6)
...
if __name__ == '__main__':
    ...
    for shopId in shop_list:
        try:
            pool.apply_async(get_shop_waite, (shopId,))  # 维持执行的进程总数为processes，当一个进程执行完毕后会添加新的进程进去
    ...
    for i in range(5):
        ...
        if len(lost_storeId_list) > 7:
            for storeId in lost_storeId_list:
                try:
                    pool.apply_async(get_shop_waite, (storeId, 1))
                    # 传入1,启用代理,传入2,不启动代理
        ...
        if len(lost_storeId_list) > 7:
            ...
            for storeId in lost_storeId_list:
                try:
                    ...
                    pool.apply_async(get_shop_waite, (storeId, 2))
                except Exception as e:
                    # print(e)
                    pass
        else:
