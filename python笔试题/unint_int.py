...
总计需要内存2.5亿 * 2bit，约等于0.6G。算法的过程大致如下：'''
"""
...
key = int(2.5 * (10 ** 8))
mark = np.zeros((key, 2), dtype=np.bool)


# 初始化一个(2.5*10^8) * 2 bool数组，并且初始化为False，对于每一个整数，使用两个bit来表示它出现的次数： 00：出现0次,01：出现一次,11: 出现多次


def get_unique_int():
	with open('bigdata_int.txt') as file:
		# big_data:原始的2.5亿个整数大文件
		for num in file:
			# print(num)
			num = int(num.replace('\n', ''))
			if mark[num][0] == False and mark[num][1] == False:  # 这个数第一次出现。那么更改属于它的2个bit
				...
				mark[num][1] = True
	with open('bigdata_int.txt') as file:  # bigdata:原始的2.5亿个整数大文件
		for num in file:
			num = int(num.replace('\n', ''))
